...
class MS_AI_Behaviour : public dp88_AI_Tank_Offensive, public Pathfind_Help_Implementer
{
	...
	void Post_Created(GameObject *obj)
	{
		...
		int weaponMaxRange = max(primary_maxRange, secondary_maxRange);
		float defSightRange = obj->As_SmartGameObj()->Get_Definition().Get_Sight_Range();
		float neededSightRange = max( max(weaponMaxRange, defSightRange), 100 ) + 5; // +5: Sight range needs to be larger than the weapon max range
		...
		float neededArc = obj->As_SoldierGameObj() ? 135.0f : 360.0f;
		if (obj->As_SmartGameObj()->Get_Definition().Get_Sight_Arc() < neededArc)
	...
	void AttackTarget(GameObject *obj, GameObject *target) override
	{
		...
		params.AttackForceFire = m_bTargetPrimaryFire ? isPrimaryAmmoTurning : isSecondaryAmmoTurning;

		// COPIED ORIGINAL FUNCTION, MODIFIED FROM HERE
		// Soldiers crouch with some probability
		bool moveCrouched = false;
		if (bPersueTarget && obj->As_SoldierGameObj())
		{
		...
		float currentDistance2 = Vector3::Distance(Commands->Get_Position(obj), Commands->Get_Position(target));
		...
		else if (bPersueTarget)
		{
			...
			if (isTurretRotationLimited)
			{
				...
				if ((angle < turretTurnMin || angle > turretTurnMax) && (!(m_bTargetPrimaryFire ? isPrimaryAmmoTurning : isSecondaryAmmoTurning) || (abs(angle) > DEG2RAD(20) && Vector3::Distance(Commands->Get_Position(obj), targetPos) > radius + 10.0f)))
					needsRotating = true;
			}
		...
		if (isVTOL)
		{
			...
			auto helperPos = Commands->Get_Position(obj);
			helperPos.Z -= 15;
	...
	void Handle_Target_Reaction_Think(GameObject *obj, GameObject *target)
	{
		...
		if (!gettingAround && !deployOrderOverride)
		{
			float distFromTarget = getDistance(obj, target);
			float maxRange = (float)((m_bTargetPrimaryFire) ? primary_maxRange : secondary_maxRange);
			bool inRange = maxRange > distFromTarget;
			bool inPrefRange = GetPreferredAttackRange(obj, target) >= distFromTarget;
			...
			if (obj->As_SoldierGameObj() || (state != GOING_TO_REFILL && state != GOING_TO_SWITCH_OBJECTIVE && (!isTurretRotationLimited || overridingTask))) {
				...
				else if (objectiveObj && (m_pCurrentObjective->GetRange() + 1) < getDistance(obj, m_pCurrentObjective->GetGameObject()))
				{
					...
					auto objectivePos = Commands->Get_Position(objectiveObj);
...
class MS_AI_Team_Controller : public ScriptImpClass
{
	...
	GameObject *Find_Closest_AI_Building(const Vector3& position, int team)
	{
	...
	void Check_Explosives()
	{
		...
		for (auto node = GameObjManager::BeaconGameObjList.Head(); node; node = node->Next())
		{
			...
			if (!isValidTarget)
			{
				...
				auto bld = Find_Closest_AI_Building(objPos, team);
...
class MS_AI_Player_Count_Controller : public ScriptImpClass
{
	...
	bool Init_Parameters(GameObject *obj)
	{
		...
		if (!checkedError.Is_Empty())
		{
			errorMessage.Format("%s", checkedError.Peek_Buffer());
			valid = false;
		}
	...
	*/
	...
	void Spawn_Object(GameObject *obj)
	{
		...
		*/
		...
		soldier->As_SoldierGameObj()->Set_Bot_Tag(botName);
	...
	const char* Choose_Unit(const char *checkPresetList, bool isInfantry)
	{
		...
		else
		{
			...
			for (int i = 0; i < fullList.Count(); i++)
			{
				namedList.Add(DefinitionMgrClass::Find_Definition(fullList[i], false)->Get_Name());
			}
		}
